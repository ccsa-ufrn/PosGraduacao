...
API_KEYS_FILE_PATH = '../env/api_keys.json'
...
ID_GOOGLE_MAPS = 'google_maps_js_api'



def get_api_credentials_dict(id_key):
    ...
    Args:
        key_id (str): an identificator for the wanted API inside the found json
                      ...
                      (PS: avoid using literal strings, use the local constants ID_*)
    ...
    try:
        with open(API_KEYS_FILE_PATH, 'r') as keys_file:
            keys_file_content = keys_file.read()
    except FileNotFoundError:
        logging.error('Failed to find ' + API_KEYS_FILE_PATH)
        return None
    except IOError:
        logging.error('IO Error when trying to read ' + API_KEYS_FILE_PATH)
        return None
    ...
    try:
        ...
        logging.error('Failed to decode json from ' + API_KEYS_FILE_PATH)
        return None
    ...
    try:
        ...
        logging.error('Didn\'t find key \'' + id_key + '\' in dict from ' + API_KEYS_FILE_PATH)
        return None
